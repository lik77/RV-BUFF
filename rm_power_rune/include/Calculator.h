...
class Calculator {
   public:
    ...
    inline cv::Point3f getPredictRobot() { return m_predictRobot; }//返回击打的机器人坐标的三维预测
    inline cv::Point2f getPredictPixel() { return m_predictPixel; }//返回像素的二维预测位置
    inline std::pair<double, double> getPredictPitchYaw() //返回预测的pitch和yaw
    {
   ...
   private:
    ...
    void fit();
    bool fitOnce();
    std::vector<cv::Point2f> m_cameraPoints;//相机坐标系下的装甲板中心点
    std::vector<cv::Point3f> m_worldPoints;//世界坐标系下的三维装甲板中心点
    ...
    double m_distance2Target;//帧距离目标的距离当前目标与相机的距离
