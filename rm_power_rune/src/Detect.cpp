...
bool Detector::detect(const Frame& frame) {
    ...
    case RuneType::SMALL_RUNE:
        Param::ARMOR_BRIGHTNESS_THRESHOLD = 200;
        Param::MIN_ARMOR_AREA = 100;
        Param::MAX_ARMOR_AREA = 400;
        break;
        
    case RuneType::LARGE_RUNE:
        Param::ARMOR_BRIGHTNESS_THRESHOLD = 180;
        Param::MIN_ARMOR_AREA = 200;
        Param::MAX_ARMOR_AREA = 800;
        break;
        
    case RuneType::OUTPOST_RUNE:
        Param::ARMOR_BRIGHTNESS_THRESHOLD = 220;
        Param::MIN_ARMOR_AREA = 80;
        Param::MAX_ARMOR_AREA = 300;
        break;
    }
...
bool Detector::detectArmor() {
    // 1. 找到内部装甲板灯条
    std::vector<Lightline> lightlines;
    if (findArmorLightlines(m_imageArmor, lightlines, m_globalRoi, m_armorRoi) == false) {
        return false;
    }
    ...
    // 3. 根据装甲板实际物理尺寸计算外部点位
    calculateOuterPoints(m_armor);

    return true;
}

void Detector::calculateOuterPoints(Armor& armor) {
    // 计算装甲板朝向
    cv::Point2f direction = armor.m_inside.m_center - m_centerR.m_center;
    direction = direction / cv::norm(direction);
    
    // 根据物理尺寸比例计算外部点位置
    float outer_distance = Param::ARMOR_OUTSIDE_HEIGHT / Param::ARMOR_INSIDE_HEIGHT * 
                          pointPointDistance(armor.m_inside.m_center, m_centerR.m_center);
    
    // 设置外部点位置
    armor.m_outside.m_center = m_centerR.m_center + direction * outer_distance;
    
    // 计算外部宽度
    float outer_width = armor.m_inside.m_width * (Param::ARMOR_OUTSIDE_WIDTH / Param::ARMOR_INSIDE_WIDTH);
    
    // 设置外部四个角点
    cv::Point2f perpendicular(-direction.y, direction.x);
    armor.m_tlOut = armor.m_outside.m_center + perpendicular * (outer_width/2);
    armor.m_trOut = armor.m_outside.m_center - perpendicular * (outer_width/2);
    // ...设置其他外部点
}
